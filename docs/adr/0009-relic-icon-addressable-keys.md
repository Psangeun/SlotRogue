...
유물은 v23 기준 80종에서 v30 별조각 상점 55종으로 재편되었다. 초기에는 16개 Sprite 시트만 준비되어 있었지만, v30 전용 84 x 84 Sprite 56개 시트가 추가되었다. 유물 수와 아이콘 수가 계속 바뀔 수 있으므로 View나 Prefab에 유물 ID별 Sprite 참조를 직접 늘리면 화면마다 매핑이 중복되고, Addressables 또는 AssetBundle로 이전할 때 로드 책임이 분산된다.
...
- 현재 v30 아이콘 시트는 `Relic Sheet 300` 주소 하나로 등록한다.
- 시트 내부 Sprite는 Addressables subasset 문법인 `Relic Sheet 300[SpriteName]`으로 참조한다.
- 키 문자열은 `RelicIconKeys`에 모은다. v30 유물 55종은 카탈로그 순서대로 `Slot00`~`Slot54`를 사용하고, `Slot55`는 여분으로 둔다.
...
- **전용 아이콘이 완성될 때까지 단일 기본 Sprite 유지** — 준비된 시트 아트를 활용할 수 없고 이후 데이터 계약을 다시 바꿔야 하므로 제외했다.

## Consequences

- 현재는 v30 유물 55종이 각각 고유 아이콘 키를 사용한다.
- 아이콘을 추가해도 View·ViewModel·Prefab 구조는 바뀌지 않는다.
- Addressables 키가 누락되면 공용 기본 아이콘으로 대체되고 경고가 한 번 기록된다.
- 구 16종 유물 시트는 Editor에서 참조 제거와 시각 검증을 마치기 전까지 Addressables 엔트리로 남길 수 있다. 이 시트는 코드에서 `Resources.Load*`로 읽지 않는다.
