...
namespace SlotRogue.UI.GameFlow
{
    ...
    /// </summary>
    public class BattleSceneHost : MonoBehaviour, IBattleSceneController
    {
        ...
        [SerializeField] private ShopDescriptionView _shopDescriptionView;
        ...
        public RunBattleTutorialTargets TutorialTargets =>
            _view != null ? _view.TutorialTargets : RunBattleTutorialTargets.Empty;
        ...
        {
            _tutorialTargetSelectionBlocked = blocked;
        ...
        private async UniTask BeginBattleAsync(CancellationToken cancellationToken)
        {
            ...
            ApplyTutorialInputBlocks();
        }

        private void ApplyTutorialInputBlocks()
        {
            ...
            _battleFlowController.SetSpinInputBlocked(_tutorialSpinBlocked);
            _battleFlowController.SetTargetSelectionBlocked(_tutorialTargetSelectionBlocked);
        }
        ...
        private BattleFlowController CreateBattleFlowController(CancellationToken cancellationToken)
        {
            ...
            var screenController = new BattleScreenController(_view, _shopDescriptionView);
        ...
        private RunEncounterRoster CreateEncounterRoster()
        {
            ...
            if (_encounterTable == null)
            {
                throw new InvalidOperationException(
                    "[BattleSceneHost] EncounterTable is missing. " +
        ...
        private WaveSchedule CreateWaveSchedule()
        {
            ...
            {
                throw new InvalidOperationException(
                    "[BattleSceneHost] WaveScheduleDefinition is missing. " +
            ...
            {
                throw new InvalidOperationException(
                    $"[BattleSceneHost] WaveScheduleDefinition '{_waveScheduleDefinition.name}' is invalid:" +
        ...
        private EncounterBalanceConfig CreateEncounterBalanceConfig()
        {
            ...
            {
                throw new InvalidOperationException(
                    "[BattleSceneHost] EncounterBalanceSettings is missing. " +
        ...
        private bool ValidateSceneReferences()
        {
            ...
            {
                Debug.LogError("[BattleSceneHost] Battle screen view is incomplete.");
                return false;
            }

            if (!_view.HasRequiredControls())
            {
                Debug.LogError("[BattleSceneHost] Battle action controls are incomplete.");
                return false;
            }

            bool isValid = true;
            isValid &= ValidateReference(_shopDescriptionView, nameof(_shopDescriptionView));
        ...
        {
            ValidateReference(_view, nameof(_view));
            ValidateReference(_shopDescriptionView, nameof(_shopDescriptionView));
            ValidateReference(_floatingTextLayerView, nameof(_floatingTextLayerView));

            ValidateReference(_spinLeverView, nameof(_spinLeverView));
            ValidateReference(_slotMachineFrameView, nameof(_slotMachineFrameView));
            ValidateReference(_slotPresentationManager, nameof(_slotPresentationManager));
        }
        ...
        private bool ValidateReference(UnityEngine.Object reference, string fieldName)
        {
            ...
            Debug.LogError($"[BattleSceneHost] Required scene reference '{fieldName}' is missing.", this);
            return false;
        }
        ...
        private async UniTask EnsureSlotPatternCatalogAsync(CancellationToken cancellationToken)
        {
                ...
                _slotPatternCatalogHandle.Result == null)
            {
                ...
                Debug.LogError(
                    $"[BattleSceneHost] Addressable '{SlotPatternCatalog.Address}' load failed: {reason}. " +
        ...
        private async UniTask EnsureSlotSymbolSpritesAsync(CancellationToken cancellationToken)
        {
            ...
            if (_loadedSlotSpinSymbolSprites == null)
            {
            ...
            if (_loadedSlotHighlightSymbolSprites == null)
            {
                _loadedSlotHighlightSymbolSprites = await LoadSpriteTableAsync(
                    SlotSymbolIconKeys.HighlightSpriteKeys,
                    "highlight slot symbols",
                    cancellationToken);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            for (int index = 0; index < keys.Length; index++)
            {
                ...
                {
                    Debug.LogWarning(
                        $"[BattleSceneHost] Addressable {tableName} sprite '{keys[index]}' load failed. " +
        ...
        private void ApplySlotSymbolSprites()
        {
            ...
            _view?.SetSlotHighlightSymbolSprites(_loadedSlotHighlightSymbolSprites);
        }
